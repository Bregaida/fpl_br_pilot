...
/**
 * Entidade JPA que representa um aerÃƒÂ³dromo no banco de dados.
 */
@Entity
...
public class AerodromoEntity extends PanacheEntityBase {
    ...
    /**
     * Converte esta entidade para o domÃƒÂ­nio Aerodromo.
     */
    ...
    /**
     * Cria uma nova entidade a partir de um domÃƒÂ­nio Aerodromo.
     */
        ...
        return AerodromoEntity.builder()
                ...
                .pistas(aerodromo.getPistas() != null ? 
                        aerodromo.getPistas().stream()
                                .map(p -> PistaEntity.fromDomain(p, null)) // O aerÃƒÂ³dromo serÃƒÂ¡ definido apÃƒÂ³s a criaÃƒÂ§ÃƒÂ£o
                                .collect(java.util.stream.Collectors.toList()) : null)
                .frequencias(aerodromo.getFrequencias() != null ? 
                        aerodromo.getFrequencias().stream()
                                .map(f -> FrequenciaEntity.fromDomain(f, null)) // O aerÃƒÂ³dromo serÃƒÂ¡ definido apÃƒÂ³s a criaÃƒÂ§ÃƒÂ£o
                                .collect(java.util.stream.Collectors.toList()) : null)
